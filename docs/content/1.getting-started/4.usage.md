...
<template>
  <UButton @click="counter++">Count: {{ counter }}</UButton>
</template>
```
...
While developing, open Nuxt DevTools and check the **Realtime** tab to see active connections, subscribed channels/storage keys, live storage values, and a scrolling event log. See [DevTools](/getting-started/devtools) for details.
...
<template>
  <div>
    <!-- Connection status -->
    <div class="mb-4">
      Status: <UBadge :color="connected ? 'success' : 'error'" variant="subtle">{{ status }}</UBadge>
    </div>
    ...
    <!-- Typing indicator -->
    <p v-if="typingUsers.size > 0" class="text-sm text-muted">
    ...
    <form @submit.prevent="sendMessage" class="flex gap-2">
      <UInput
        ...
        @input="onTyping"
      />
      <UButton type="submit" :disabled="!connected">
        Send
      </UButton>
    </form>
  </div>
