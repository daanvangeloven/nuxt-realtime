...
<template>
  <Transition name="slide">
    <UAlert
      ...
      color="warning"
      variant="solid"
      class="fixed top-0 inset-x-0 rounded-none text-center"
      :title="status === 'reconnecting' ? 'Reconnecting to server...' : 'You are offline. Some features may not work.'"
    />
  </Transition>
</template>
...
<template>
  <div class="flex items-center gap-2">
    ...
    <UButton v-if="connected" color="error" @click="disconnect">
      Go Offline
    </UButton>
    <UButton v-else color="success" @click="connect">
      Reconnect
    </UButton>
  </div>
</template>
...
<template>
  <div class="space-y-2">
    <p>Counter: {{ counter }}</p>
    <UButton :disabled="!connected" @click="counter++">
      Increment
    </UButton>
