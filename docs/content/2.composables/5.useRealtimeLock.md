...
<template>
  <button v-if="!locked" @click="claim()">Start editing</button>
  <button v-else-if="ownedByMe" @click="release()">Done editing</button>
  <p v-else>Someone else is editing this</p>
</template>
```
...
<template>
  <p v-if="locked && !ownedByMe">Locked by {{ ownerInfo.name }}</p>
</template>
```
...
<template>
  <div>
    <p v-if="locked && !ownedByMe" class="text-yellow-500">
      {{ ownerInfo.name }} is editing this record
    </p>

    <input v-model="record.title" :disabled="locked && !ownedByMe" />

    <button v-if="!ownedByMe" :disabled="locked" @click="startEditing">
      Edit
    </button>
