...
<template>
  <USkeleton v-if="data.loading.value" class="h-6 w-40" />
  <div v-else>{{ data }}</div>
</template>
```
...
<template>
  <div class="flex items-center gap-4">
    <UButton icon="i-lucide-minus" square variant="soft" @click="counter--" />
    <span class="w-8 text-center tabular-nums">{{ counter }}</span>
    <UButton icon="i-lucide-plus" square variant="soft" @click="counter++" />
  </div>
</template>
```
...
<template>
  <UInput v-model="query.value" icon="i-lucide-search" placeholder="Search..." />
</template>
```
...
<template>
  <div class="space-y-2">
    <UTextarea v-model="doc.value" autoresize class="w-full" />
    <div class="flex items-center gap-2">
      <UBadge v-if="doc.isDirty.value" color="warning" variant="subtle">
        Unsaved changes
      </UBadge>
      <UButton :disabled="!doc.isDirty.value" @click="doc.sync()">
        Save
      </UButton>
    </div>
  </div>
...
<template>
  <div class="space-y-2">
    ...
    <UTextarea v-model="form.message" placeholder="Message" />
  </div>
</template>
```
...
const onlineUsers = useRealtimeState<User[]>('online-users', [])
...
<template>
  <UAvatarGroup>
    <UAvatar v-for="user in onlineUsers" :key="user.id" :text="user.name[0]" />
  </UAvatarGroup>
</template>
```
...
<template>
  <div>
    <USkeleton v-if="data.loading.value" class="h-6 w-40" />
    <p v-else>{{ data.length }} items loaded</p>
  </div>
</template>
```
