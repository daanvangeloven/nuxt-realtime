...
│  unstorage + Redis      │  │  RealtimePubSub                │
│  Data persistence       │  │  Cross-server messaging only   │
└─────────────────────────┘  └────────────────────────────────┘
```
...
Each `RealtimePubSub` instance generates a UUID when it starts up (`crypto.randomUUID()`). Every published message includes this as `origin`.
...
```
Nitro plugin starts
  └─> new RealtimePubSub(redisOpts)
        ├─> reactiveRedisDriver({ pubsub })
        │     registers handler on nuxt-realtime:watch
        └─> pubsub.subscribe('nuxt-realtime:events', handler)
              registers handler on nuxt-realtime:events

Nitro plugin shuts down  (nitroApp.hooks.hook('close', ...))
  └─> unwatch()                  <- storage driver watch removed
  └─> unsubscribeEvents()        <- event relay handler removed
  └─> pubsub.dispose()           <- pub + sub connections closed
```
...
A warning gets logged at startup so it's obvious what's happening:

```
[nuxt-realtime] No Redis pub/sub configured.
Cross-server event sync is disabled...
```

No client-side changes are needed when switching between single and multi-server setups.
